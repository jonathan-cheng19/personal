(function () {
    if (!window.THREE) {
        console.error("Three.js is required for the tiny house studio to run.");
        return;
    }
    if (!THREE.OrbitControls) {
        console.error(
            "OrbitControls helper missing. Include https://unpkg.com/three@0.155.0/examples/js/controls/OrbitControls.js before scripts/tiny-house.js."
        );
        return;
    }
    if (!THREE.Sky) {
        console.error(
            "Sky helper missing. Include https://unpkg.com/three@0.155.0/examples/js/objects/Sky.js before scripts/tiny-house.js."
        );
        return;
    }

    // Flag so the HTML shell can detect whether the script executed successfully.
...
function updateVariantCountLabel() {
    if (ui.variantCount) {
        ui.variantCount.value = "1";
        ui.variantCount.disabled = true;
    }
    if (ui.variantLabel) {
        const available = state.designs.length;
        ui.variantLabel.textContent = available ? `${available} layout ready` : "Single layout mode";
    }
    if (ui.headerVariantBadge) {
        const badgeValue = state.designs.length || state.totalGenerated || 0;
...
async function generateDesigns() {
    ...
    setSimulationStatus("Synthesizing intelligent layout variant…");
    ...
    try {
        ...
        state.designs.push({ design, seed, analytics });
        state.totalGenerated = state.designs.length;
        updateGenerationProgress(1, 1);
        await yieldToFrame();
        ...
        completeGenerationProgress({
            message: "Layout ready. Inspect the generated design.",
            success: true,
        });
        setSimulationStatus(`Generated layout ${design.id}.`);
    } catch (error) {
        console.error("Generation error", error);
        setSimulationStatus("Generation failed. Adjust parameters and try again.");
        if (ui.designList) {
            ui.designList.innerHTML =
                '<div class="design-item error" role="alert">Unable to generate layout. Please review inputs and retry.</div>';
        }
        completeGenerationProgress({
